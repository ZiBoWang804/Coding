...
export function SpotCard({ spot }: { spot: RuralSpotSeed }) {
  ...
  return (
    <Link
      href={`/spots/${spot.id}`}
      className="group overflow-hidden rounded-3xl border border-brand-100 bg-white shadow-card transition hover:-translate-y-1 hover:border-brand-200 hover:shadow-xl"
    >
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="rounded-2xl bg-brand-50/70 p-3">
          <div className="text-xs tracking-wide text-brand-700">附近酒店参考</div>
          <div className="mt-1 text-sm font-medium text-brand-900">{hotelSummary.title}</div>
