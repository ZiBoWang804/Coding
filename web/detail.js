...
function upgradeTopicFollowAction(state, clusterId) {
  ...
  const cluster = findCluster(state, clusterId);
  const detailActions = document.querySelector("#detailActions");
  const existingButton = detailActions?.querySelector("button.detail-action.primary");
  if (!cluster || !existingButton) {
    return;
  }

  const isTracked = state.tracked_topic?.cluster_id === cluster.id;
  const replacement = existingButton.cloneNode(true);
  replacement.disabled = isTracked;
  replacement.textContent = isTracked ? "当前已关注" : "设为当前关注主题";

  if (!isTracked) {
    replacement.addEventListener("click", async () => {
      const previousLabel = replacement.textContent;
      replacement.disabled = true;
      replacement.textContent = "正在设为关注...";
      try {
        const nextState = await runtimeBridge.followCluster(cluster.id);
        renderTopicPage(nextState, cluster.id);
        upgradeTopicFollowAction(nextState, cluster.id);
      } catch (error) {
        console.error(error);
        window.alert(`设置关注失败：${error.message}`);
        replacement.disabled = false;
        replacement.textContent = previousLabel;
      }
    });
  }

  existingButton.replaceWith(replacement);
}
...
async function init() {
  try {
    ...
    const page = document.body.dataset.page;
    const params = new URLSearchParams(window.location.search);
    const preferredClusterId = page === "topic" ? params.get("cluster") ?? "" : null;
    const state = await runtimeBridge.loadState(preferredClusterId);
    ...
    if (page === "topic") {
      ...
      renderTopicPage(state, clusterId);
      upgradeTopicFollowAction(state, clusterId);
    }
  } catch (error) {
    console.error(error);
    const title = document.querySelector("#detailTitle");
