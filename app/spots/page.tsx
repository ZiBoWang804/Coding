      ...
      {user ? (
        <SearchRecorder
          payload={{
            ...
            resultIds: spots
              .slice(0, 10)
              .map((spot: RuralSpotSeed) => spot.id || "")
              .filter(Boolean)
          }}
        />
      ) : null}
        ...
        <aside className="h-fit rounded-3xl border border-brand-100 bg-white p-5 shadow-card">
          ...
          <form className="mt-4 space-y-4">
            <input
              ...
              placeholder="搜索名称、城市或简介"
              className="w-full rounded-2xl border border-brand-100 px-4 py-3 text-sm"
            />
            <select name="province" defaultValue={params.province} className="w-full rounded-2xl border border-brand-100 px-4 py-3 text-sm">
              ...
              {filters.provinces.map((item: string) => (
            ...
            <select name="tag" defaultValue={params.tag} className="w-full rounded-2xl border border-brand-100 px-4 py-3 text-sm">
              ...
              {filters.tags.map((item: string) => (
                <option key={item} value={item}>
                  {item}
                </option>
              ))}
            </select>
            <button type="submit" className="w-full rounded-full bg-brand-700 px-4 py-3 text-sm text-white">
              应用筛选
            </button>
          </form>
          ...
          {user && personalized.length > 0 ? (
            <div className="mt-6 rounded-2xl bg-sand p-4">
              ...
              <div className="mt-3 space-y-2 text-sm text-slate-600">
                {personalized.slice(0, 3).map((spot: RuralSpotSeed) => (
        ...
        <section>
          <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">点击任一景点即可进入详情、社区、打卡和导航入口。</p>
            </div>
          </div>
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {spots.map((spot: RuralSpotSeed) => (
