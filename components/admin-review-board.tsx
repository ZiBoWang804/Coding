...
export function AdminReviewBoard({ initialItems }: { initialItems: SpotSubmissionItem[] }) {
  ...
  async function review(id: string, decision: "APPROVED" | "REJECTED") {
    ...
    setItems((current) =>
      current.map((item) =>
        item.id === id
          ? { ...item, status: decision, reviewerNotes: notes[id] || item.reviewerNotes }
          : item
      )
    );
  }
  ...
  return (
    <div className="space-y-4">
      {items.length === 0 ? <p className="text-sm text-slate-500">目前没有待处理投稿。</p> : null}
      {items.map((item) => (
        <div key={item.id} className="rounded-2xl border border-brand-100 bg-white p-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-1 text-sm text-slate-500">
                {[item.province, item.city, item.district].filter(Boolean).join(" / ")} / 提交人：{item.user.nickname}
              </div>
          ...
          <textarea
            ...
            placeholder="审核备注，例如：建议补充停车信息或更新图片来源。"
            className="mt-3 min-h-24 w-full rounded-2xl border border-brand-100 px-4 py-3 text-sm"
          />
