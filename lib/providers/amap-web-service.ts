...
export async function getAmapWeather(adcode: string, travelDate: string) {
  ...
  const selected = forecastPayload?.forecasts?.[0]?.casts?.find((cast) => cast.date === travelDate) ?? forecastPayload?.forecasts?.[0]?.casts?.[0];
  if (selected) {
  ...
  const steps = (path?.steps ?? [])
    .map((step) => {
      ...
      const detail = compact([
        instruction,
        road ? `道路：${road}` : null,
        step.orientation ? `方向：${step.orientation}` : null
      ]).join("；");
