...
export function loadSeedSpots(): RuralSpotSeed[] {
  ...
  cache = rows.map((row, index) => ({
    ...
    lastVerifiedAt: String(row.lastVerifiedAt ?? "") || null,
    ...
    ticketBookingUrl: String(row.ticketBookingUrl ?? "") || null,
    hotelBookingUrl: String(row.hotelBookingUrl ?? "") || null,
    gaodeNavigationUrl: String(row.gaodeNavigationUrl ?? "") || null,
    lodgingSummary: String(row.lodgingSummary ?? "") || null,
    lodgingPriceMin: parseNumber(row.lodgingPriceMin) ?? null,
    lodgingPriceMax: parseNumber(row.lodgingPriceMax) ?? null,
